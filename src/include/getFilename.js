...
export default function getFilename(objectState) {
      ...
      if (!objectState.GUID) return `${Constants.GLOBAL_FILENAME}.${Constants.SCRIPT_EXTENSION}`;
      if (objectState.GUID === "-1") return `${Constants.GLOBAL_FILENAME}.${Constants.SCRIPT_EXTENSION}`;
      const nickname = objectState.Nickname ?? "";
      const parsedName = nickname.trim().replaceAll(/[ ]/g, "_");

      if (parsedName === "") return `${objectState.GUID}.${Constants.SCRIPT_EXTENSION}`;
      else return Path.join(parsedName, `${objectState.GUID}.${Constants.SCRIPT_EXTENSION}`);
};